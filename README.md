...
### Once the data is deemed fit to be used, the data is first embed using a low-cost light-weight model <q>text-embedding-3-small</q>. In order to improve the data retrieve time, the data is will be vectorized, and then the vectors will be stored in a vector database. <br> <br> We are using Pinecone for this project, the final data will be upsert into that. Before beginnning with the code kindly go through the steps to create a vector database and create an index where your data will be stored. 
